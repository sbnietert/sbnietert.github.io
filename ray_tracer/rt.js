...
(function (RT) {
    var SceneObject = (function () {
        ...
        SceneObject.prototype.getMaterialLighting = function (point) {
...
(function (RT) {
    ...
    var SimpleGeometricObject = (function (_super) {
        ...
        SimpleGeometricObject.prototype.obstructsShadowRay = function (shadowRay, distToLight) {
            var int = new BasicRayIntersection();
            return this.intersectsRay(shadowRay, int) && int.t < distToLight;
        };
...
(function (RT) {
    ...
    var SolidColorMaterial = (function () {
        function SolidColorMaterial(lighting) {
            this.lighting = lighting;
        }
        SolidColorMaterial.prototype.getLighting = function (geo, point) {
            return this.lighting;
        };
        return SolidColorMaterial;
    })();
    RT.SolidColorMaterial = SolidColorMaterial;
})(RT || (RT = {}));
/// <reference path="Vector.ts" />
/// <reference path="GeometricObject.ts" />
var RT;
(function (RT) {
    var AxisAlignedBox = (function (_super) {
        ...
        function AxisAlignedBox(A, B) {
            _super.call(this);
            this.id = ++AxisAlignedBox.count;
            AxisAlignedBox.instances.push(this);
            if (A.x > B.x) {
                this.ur = A;
                this.ll = B;
            }
        ...
        AxisAlignedBox.prototype.intersectsRay = function (ray, int) {
            ...
            if (tmax < 0 || tmin > tmax)
                return false;
            int.t = tmin;
            int.geometricData = tminIndex;
            return true;
        };
        AxisAlignedBox.isBetween = function (x, a, b) {
        ...
        AxisAlignedBox.count = -1;
        AxisAlignedBox.instances = [];
...
(function (RT) {
    var Cylinder = (function (_super) {
        ...
        Cylinder.prototype.intersectsRay = function (ray, int) {
...
(function (RT) {
    var Sphere = (function (_super) {
        ...
        Sphere.prototype.intersectsRay = function (ray, int) {
...
(function (RT) {
    var Triangle = (function (_super) {
        ...
        Triangle.prototype.intersectsRay = function (ray, int) {
...
(function (RT) {
    ...
    var SceneGraphNode = (function () {
        function SceneGraphNode(type, value, boundingShape) {
    ...
    RT.SceneGraphNode = SceneGraphNode;
})(RT || (RT = {}));
    ...
    var MengerSponge = (function () {
        ...
        MengerSponge.prototype.addBox = function (ll, ur, sceneGraph, depth) {
            ...
            if (depth == 0) {
                ...
                var node = new RT.SceneGraphNode(nodeType, box);
            ...
            for (var x = 0; x < 3; x++) {
                for (var y = 0; y < 3; y++) {
                    for (var z = 0; z < 3; z++) {
                        ...
                        var collectionNode = new RT.SceneGraphNode(nodeType, subGraph, boundingBox);
...
(function (RT) {
    var Mesh = (function () {
        ...
        Mesh.getBoundingBox = function (triangles) {
            ...
            for (var _i = 0; _i < triangles.length; _i++) {
                ...
                for (var _a = 0, _b = [tri.A, tri.B, tri.C]; _a < _b.length; _a++) {
                    var vertex = _b[_a];
                    if (vertex.x < min.x)
                        min.x = vertex.x;
                    if (vertex.x > max.x)
        ...
        Mesh.prototype.groupTriangles = function (triangles, parent) {
            if (triangles.length <= 2) {
                for (var _i = 0; _i < triangles.length; _i++) {
                    var tri = triangles[_i];
                    var obj = new RT.SceneObject(tri, this.material);
                    var node = new RT.SceneGraphNode(0 /* Object */, obj);
                    parent.value.push(node);
                }
            }
            else {
                var ref = triangles[0].A;
                triangles.sort(function (a, b) {
                    return a.A.minus(ref).length() - b.A.minus(ref).length();
                });
                var closest = triangles.splice(0, Math.floor(triangles.length / 2));
                var closestNode = new RT.SceneGraphNode(1 /* Collection */, [], Mesh.getBoundingBox(closest));
                parent.value.push(closestNode);
                this.groupTriangles(closest, closestNode);
                var furthest = triangles;
                var furthestNode = new RT.SceneGraphNode(1 /* Collection */, [], Mesh.getBoundingBox(furthest));
                parent.value.push(furthestNode);
                this.groupTriangles(furthest, furthestNode);
            }
        };
        Mesh.prototype.generateSceneGraphFromTriangles = function (triangles, material) {
            this.material = material;
            var parentNode = new RT.SceneGraphNode(1 /* Collection */, [], Mesh.getBoundingBox(triangles));
            this.groupTriangles(triangles, parentNode);
            return [parentNode];
        };
        Mesh.prototype.generateSceneGraph = function (vertices, faceVertexCounts, faceVertexIndices, normalInterpolation, material) {
            ...
            return this.generateSceneGraphFromTriangles(triangles, material);
        };
        Mesh.prototype.generateSceneGraphFromData = function (data, scene) {
            ...
            return this.generateSceneGraphFromTriangles(triangles, scene.materials[data.material]);
        };
        return Mesh;
    })();
    RT.Mesh = Mesh;
...
(function (RT) {
    var SphereHelix = (function () {
        ...
        SphereHelix.generateBoundingBox = function (spheres) {
            var min = new RT.Vector(Infinity, Infinity, Infinity);
            var max = new RT.Vector(-Infinity, -Infinity, -Infinity);
            for (var _i = 0; _i < spheres.length; _i++) {
                var sphere = spheres[_i];
                var points = new Array(6);
                for (var i = 0; i < 6; i++)
                    points[i] = sphere.center.copy();
                points[0].x += sphere.radius;
                points[0].x -= sphere.radius;
                points[0].y += sphere.radius;
                points[0].y -= sphere.radius;
                points[0].z += sphere.radius;
                points[0].z -= sphere.radius;
        ...
        SphereHelix.prototype.generateSceneGraphFromData = function (data, scene) {
            var sceneGraph = [];
            var parent = new RT.SceneGraphNode(1 /* Collection */, []);
            sceneGraph.push(parent);
            ...
            for (var t = start; t < stop; t += step) {
                ...
                var sphereMat = new RT.SolidColorMaterial(lighting);
                var sphere = new RT.SceneObject(sphereGeo, sphereMat);
                spheres.push(sphereGeo);
                var node = new RT.SceneGraphNode(0 /* Object */, sphere);
                parent.value.push(node);
            }
            parent.boundingShape = SphereHelix.generateBoundingBox(spheres);
            return sceneGraph;
        };
        return SphereHelix;
    })();
        ...
        XZSurface.prototype.getVertices = function () {
            ...
            for (var i = 0; i <= this.gridSize; i++) {
                for (var j = 0; j <= this.gridSize; j++) {
                    ...
                    // top surface vertex
                    vertices[this.getIndex(i, j, 1)] = new RT.Vector(currX, this.fn(currX, currZ) + 0.1, currZ);
                }
            }
            return vertices;
        };
...
(function (RT) {
    var Scene = (function () {
        ...
        Scene.loadMaterial = function (data) {
            ...
            if (data.type == 'solid-color' || checkered) {
                var lighting = new RT.MaterialLighting();
                if (checkered)
                ...
                return new RT.SolidColorMaterial(lighting);
            }
        };
        ...
        Scene.prototype.loadSceneGraphNode = function (data) {
            ...
            return new RT.SceneGraphNode(type, value);
        };
    ...
    var RayTracer = (function () {
        ...
        RayTracer.prototype.shadeObject = function (intersection, insideObject, depth) {
            ...
            if (reflection != 0) {
                ...
                reflectedRay.origin.add(reflectedRay.direction.scaledBy(RT.EPSILON));
            ...
            if (refraction != 0) {
                ...
                refractedRay.origin.add(refractedRay.direction.scaledBy(RT.EPSILON));
...
(function (RT) {
    ...
    var SceneEditor = (function () {
        function SceneEditor(canvas) {
            var _this = this;
            this.canvas = canvas;
            this.sceneFile = SceneEditor.sceneFiles[3];
            this.sceneData = new SceneData();
            this.gui = new dat.GUI({ width: 400 });
            this.rayTracer = new Worker('rayTracerWorker.js');
            this.noImage = true;
            this.ctx = canvas.getContext('2d');
            var sceneFileController = this.gui.add(this, 'sceneFile', RT.SceneEditor.sceneFiles);
            sceneFileController.onChange(function () {
                _this.loadScene(false);
            });
            sceneFileController.name('Scene File');
        ...
        SceneEditor.prototype.addRenderButton = function () {
            this.renderButtonController = this.gui.add(this, 'render');
            this.renderButtonController.name('Render!');
        };
        SceneEditor.prototype.loadScene = function (render) {
            this.rayTracer.postMessage({
                command: 'load scene',
                data: SceneEditor.scenePathPrefix + this.sceneFile
            });
        };
        ...
        SceneEditor.scenePathPrefix = 'scenes/';
        SceneEditor.sceneFiles = [
            'mengerSponge.json',
            'models.json',
            'ripple.json',
            'sample.json'
